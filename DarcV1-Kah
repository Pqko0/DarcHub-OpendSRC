-- Pqko was here.... and will stay here
-- So, basically half of the code on this is quite shit and frankly outdated. most of my new stuff isnt in this one, meaning it has all the shit stuff like potato kick 💀💀💀💀💀💀
-- Also if u copy any of this code without my permission ur a fking skid ok?
-- and i wont lie on this but like the new darchub script for kah will be remade and will have new kick methods which will not be on here
-- and lastly this wont be updated ever again
-- Copying without leaving credits means ur a skid...
...
if _G.pqko_script then
    return print("Darc-V1 is already executed!")
end
...
local function clearLogs()
    for i=1,20 do
        local charset = {"darc", "darcv1", "darchub.site", "v1", "ontop", "darcv1ontop", "ahhdaddy", "nologz", "L", "nologgiesforyou", "dontleaveyet:))", "kicksexist", "epicantis", "nokickingforyou", "noskiddies", "skiddingL", "logznomore", "pqkowashere", "iwasstillhere", "#", "{", "}", "?", "+", "="}
        local ch2 = {"#", "{", "}", "!", "'", "@", ":", ";"}
...
local function punish(user)
    task.spawn(
        function()
            ...
            command("punish " .. user .. " " .. math.random(0, 9) .. math.random(0, 9) .. math.random(0, 9))
            command("blind " .. user .. " " .. math.random(0, 9) .. math.random(0, 9) .. math.random(0, 9))
            command("pm " .. user .. " Server protected by Darc-V1")
...
local function cMsg(m)
    if type(m)== "string" then
        command("blind all [Darc V1] " .. antiSpam())
        serverMSG("#################################")
        serverMSG("\n\n\n\nCrashed by Darc V1")
        serverMSG("\n\n\n\n\n\n\n\nMethod: " .. m)
        serverMSG("\n\n\n\n\n\n\n\n\n\n\n\n#################################")
        ...
        function()
            ...
            while true do
                ...
                for i = 1, 700 do
                    ...
                    command("freeze all [DarcV1] " .. antiSpam())
                    for i = 1, 700 do
                        for i = 1, 3 do
                            command("size all 0.3 [DarcV1] " .. antiSpam())
                        end
                        command("freeze all [DarcV1] " .. antiSpam())
                        for i = 1, 3 do
                            command("size all 9.9")
                        end
                        command("clone all [DarcV1] " .. antiSpam())
                        task.wait(0.2)
                    end
                end
            end
...
local function tempCrash(seconds)
    ...
    repeat
        task.wait(0.05)
        command("dog all [DarcV1] " .. math.random(1, 1000)  .. math.random(1, 1000))
        command("clone all [DarcV1] " .. math.random(1, 1000)  .. math.random(1, 1000))
    until eTime < os.time()
    command("undog all [DarcV1] " .. antiSpam())
    command("removeclones [DarcV1 ]")
end
...
local function glitch(plr)
    task.spawn(function()
        command("refresh " .. plr .. " [DarcV1] " .. antiSpam())
        command("setgrav " .. plr .. " -nan")
        command("trip " .. plr .. " [DarcV1] " .. antiSpam())
    end)
end
            ...
            while true do
                ...
                if cSpamToggle == true then
                    if antiSpamToggle == true then
                        command(cSpamCommand .. " [Darc V1] " .. antiSpam())
                    else
...
local function LocalPlayerChattedFunction(message)
    ...
    if cmd == prefix .. "bg" then
        if options.BanGears == true then
            options.BanGears = false
            msg("[DarcV1] Ban gears has been turned off")
        else
            options.BanGears = true
            msg("[DarcV1] Ban gears has been turned on")
        end
    end
...
local function playerChatted(player, message)
    ...
    if cmd == cPrefix .. "finger" then
        ...
        local function main(plr)
            command("unhat " .. plr .. " [DarcV1] " .. antiSpam())
    ...
    if options.BanGears == true then
        if plr.Name == Player.name and not status == 1 then
            if tableIncludes(bannedGear, item.Name) and not tableIncludes(kicked, plr.Name) then
                ...
                serverMSG("[DarcV1] Sorry, that gear is banned. " .. antiSpam())
            end
        end
    end
end
...
local function PlayerAddedFunction(plr)
    ...
    serverMSG("Server is being moderated. [Darc V1]")
    playerCreate(name)
    if options.ServerLock == true then
        kick(name)
    end
...
local function LocalPlayerCharacterAdded()
    command("god me [DarcV1] " .. antiSpam())
end
...
local function startup()
    UserInputService.WindowFocused:Connect(
        function()
            task.wait(0.3)
            afk = false
            command("unname me [DarcV1] " .. antiSpam())
            command("unff me [DarcV1] " .. antiSpam())
        end
    )
                ...
                for _,v in pairs(PlayerService:GetPlayers()) do
                    if v.Character and v.Character:FindFirstChild("Torso") and v.Character.Torso:FindFirstChild("SPINNER") then
                        if v.Name == Player.name then
                            ...
                            command("reset " .. v.Name .. " [DarcV1] " .. antiSpam())
                        end
                    end
                end
            end
		...
		while true do
			...
			for i,v in pairs(Lighting:GetChildren()) do
                ...
                if plr.whitelisted == true and
					...
					command("unpunish " .. pName .. " [DarcV1] " .. antiSpam())
				end
			end
                ...
                if plr.whitelisted == true and plr.options.antiDeath == true then
                    ...
                    if wPlr and wPlr:FindFirstChild("HumanoidRootPart") then
                        ...
                        if humanoid.Health == 0 then
                            command("reset " .. name .. " [DarcV1] " .. antiSpam())
                            command("god " .. name .. " [DarcV1] " .. antiSpam())
                        end
                    else
                ...
                for i,v in pairs(game:GetService("Players"):GetPlayers()) do 
                        ...
                        if s <= 1 or s >= 2 and v.Character:FindFirstChild("Humanoid").Health == inf then
                            if s<= 1 and options.AntiSilCrash == true then
                                command("size all 10")
                                command("reset all [DarcV1] " .. antiSpam())
                                task.wait(0)
                            else
                                command("reset " .. v.Name .. " [DarcV1] " .. antiSpam())
                            end
            ...
            if options.AntiFreeze == true then
                for i,v in pairs(PlayerService:GetPlayers()) do
                    if v.Character and v.Character:FindFirstChild("ice") then
                        command("thaw " .. v.Name .. " [DarcV1] " .. antiSpam())
                    end
                end
            end
        end
            ...
            if options.AntiMessage == true then
                if Player.PlayerGui then
                    if Player.PlayerGui:FindFirstChild("MessageGUI") then
                        command("clr")
                        serverMSG(":m is disabled! [DarcV1]")
        ...
        while true do
            ...
            if options.AntiMusic == true then
                if Kohls.Folder:FindFirstChild("Sound") then
                    command("clr")
                    serverMSG("music is disabled! [DarcV1]")
        ...
        while true do
            ...
            if options.AntiFog == true then
                if tostring(Lighting.FogEnd) == "100000" then else
                    ...
                    serverMSG("fogend is disabled [Darc V1]")
                end
                if tostring(Lighting.FogStart) == "0" then else
                    command("fogstart 0")
                    Lighting.FogStart = 0
                    serverMSG("fogstart is disabled [Darc V1]")
                end
            end
        end
    end)()
        ...
        while true do
            ...
            if options.AntiTime == true then
                if Lighting.ClockTime == 14 then else
                    ...
                    serverMSG("time is disabled [DarcV1]")
                end
            end
        end
    end)()
    ...
    --                     command("removejails [DarcV1] " .. antiSpam())
    --                     serverMSG("jails is disabled [DarcV1]")
    --                 else
    ...
    --                             command("removetools " .. p.Name .. " [DarcV1] " .. antiSpam())
    --                         end
    --                     end
    --                 end
    --             end
    ...
    task.spawn(
        function()
            if tableIncludes(permBlacklisted, Player.Name) then
                msg("[DarcV1]: I was executed by a blacklisted person.")
                while true do end
            end
            gui()
            hasPerm()
            ...
            msg("Darc V1 has loaded " .. t .. " seconds! Contact pqko for more help.")
